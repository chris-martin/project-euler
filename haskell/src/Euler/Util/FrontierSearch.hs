...
searchValues =
  dedupe . fmap nodeWrapperValue . search

searchNodes :: (Ord node, Ord value) => Conf node value -> [node]
searchNodes =
  dedupe . fmap unwrap . search

search :: (Ord node, Ord value) =>
  Conf node value -> [NodeWrapper node value]
search conf =
    nodesFromFrontier conf initialFrontier
  where
    initialFrontier = Set.fromList $ nodeWrapper conf <$> start conf

data NodeWrapper node value = NodeWrapper node value
  deriving (Eq, Show)

instance (Ord node, Ord value) => Ord (NodeWrapper node value)
  where
...
nodeWrapper conf node =
  NodeWrapper node (nodeValue conf node)

next' :: Conf node value -> node -> [NodeWrapper node value]
next' conf node =
  nodeWrapper conf <$> next conf node

type Frontier node value = Set (NodeWrapper node value)

{- |

As we replace nodes on the search frontier with their 'next' nodes, we grow the
visited space. At each step we select the frontier node with the lowest value,
so the 'nodes' are ordered by value ascending (with ties broken arbitrarily but
consistently).
