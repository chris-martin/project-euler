module Euler.Problems.Problem69 (answer) where
...
data RC = RC { toSetMap :: SetMap Int Int }

rcToList :: RC -> [(Int, [Int])]
rcToList = fmap (fmap toList) . Map.toList . SetMap.toMap . toSetMap

totient :: RC -> Int -> Int
totient rc n = n - 1 - length (SetMap.lookup n $ toSetMap rc)
