...
object Problem54 {
  ...
  trait CardAttributeCompanion[A <: CardAttribute] {
    ...
    val values: Seq[A] = (0 until characters.size) map constructFromInt
  ...
  case class Card(ordinal: Ordinal, suit: Suit)
  ...
  case class Hand(cards: Set[Card]) {
    ...
    def isStraight: Boolean = sortedOrdinals sameElements (low to high)
