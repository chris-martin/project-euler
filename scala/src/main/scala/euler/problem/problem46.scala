...
package object problem46 {
  ...
  def answer: Int = {
    ...
    while (a.isEmpty) {
      ...
      val v = node.value
      if (v > highest) {
          a = (highest+1 until v).find(isOddComposite)
          highest = v
      }
    }
  ...
  final case class SquareDoubles(i: Int) extends Cons[Int] {
    def head = 2*i*i
  ...
  final case class Base(vertical: Cons[Int], horizontal: Cons[Int]) extends Node {
  ...
  final case class Stalk(vertical: Cons[Int], horizontal: Int) extends Node {
