...
package object problem51 {

  /** A collection of primes that have the same number of digits. */
  final case class PrimeDigitGroup(members: Seq[Int]) {
  ...
  final case class PrimeValueFamily(members: immutable.SortedSet[Int]) {
  ...
  def answer: Int = smallestPrimeValueFamily(8).members.min
